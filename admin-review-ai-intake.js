(function(){
  ...
  function money(v){
    const n = Number(v || 0);
    if (!Number.isFinite(n) || n <= 0) return "ให้ระบบคำนวณอีกครั้ง";
  ...
  function injectStyle(){
    ...
    style.textContent = `
      ...
      .ai-intake-head .top{display:flex;align-items:center;justify-content:space-between;gap:10px}
    ...
    panel.innerHTML = `
      <section class="ai-intake-head">
        <div class="top">
          ...
          <span class="ai-intake-badge">${actionable.length} รายการ</span>
        </div>
      </section>
      ${actionable.map(cardHtml).join("")}
    `;
    const alertBox = byId("approvalAlert");
    if (alertBox) {
      alertBox.style.display = "block";
      const old = text(alertBox.textContent);
      const aiLine = `🤖 LINE AI: พร้อมเพิ่มงาน ${ready} • ต้องถามเพิ่ม ${need} • แอดมินตอบเอง ${admin}`;
      if (!old.includes("LINE AI")) alertBox.innerHTML = `${esc(aiLine)}${old ? `<br>${alertBox.innerHTML}` : ""}`;
      else alertBox.innerHTML = esc(aiLine);
    }
  ...
  async function loadAiIntakes(){
    try {
      ...
      const data = await apiFetch("/admin/ai-office/booking-intakes?status=open&limit=80");
  ...
  async function fetchIntake(id){
    const data = await apiFetch(`/admin/ai-office/booking-intakes/${Number(id)}`);
  ...
  async function copyIntake(id, openAdd){
    ...
    if (!intake) return showToast("ไม่พบรายการ AI", "error");
  ...
  async function postAction(id, action, body){
    await apiFetch(`/admin/ai-office/booking-intakes/${Number(id)}/${action}`, { method:"POST", body:JSON.stringify(body || {}) });
  ...
  function bindActions(){
    document.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (close) {
          if (!confirm("ปิดรายการ AI นี้?")) return;
          return postAction(close.getAttribute("data-ai-close"), "close", { admin_note:"ปิดจากหน้างานจอง" });
        }
      } catch (err) {
        showToast(err.message || "จัดการรายการ AI ไม่สำเร็จ", "error");
      }
    });
  }
