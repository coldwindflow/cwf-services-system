...
async function loadIncomeSummary() {
  ...
  try {
    ...
    const url = `${API_BASE}/tech/income_summary${u ? `?username=${encodeURIComponent(u)}&` : '?'}v=contract-v10`;
    try { localStorage.removeItem('__cwf_income_cache__'); localStorage.removeItem('__cwf_income_cache_v9__'); localStorage.removeItem('__cwf_income_cache_v10__'); } catch {}
    ...
    try {
      localStorage.setItem('__cwf_income_cache_v10__', JSON.stringify({
        ts: Date.now(),
    ...
    try {
      const c = JSON.parse(localStorage.getItem('__cwf_income_cache_v10__') || 'null');
...
async function loadIncomeTodayMonthFast(){
  ...
  try{
    const u = _bestEffortUsername();
    const url = `${API_BASE}/tech/income_today_month${u ? `?username=${encodeURIComponent(u)}&` : '?'}v=contract-v10`;
    const res = await fetch(url, { credentials:'include', cache:'no-store' });
    const data = await res.json();
    if (!data || !data.ok) throw new Error(data?.error || 'LOAD_FAILED');
...
async function loadNextPeriodEstimate(){
  ...
  try{
    const res = await fetch(`${API_BASE}/tech/income_next_period_estimate?v=contract-v10`, { credentials:'include', cache:'no-store' });
    const data = await res.json();
    if (!data || !data.ok) throw new Error(data?.error || 'LOAD_FAILED');
...
async function loadOutstandingTotal(){
  ...
  try{
    ...
    const cache = (()=>{ try{return JSON.parse(localStorage.getItem('__cwf_income_cache_v10__')||'null');}catch{return null;} })();
    if (!cache || typeof cache !== 'object') {
      await loadIncomeSummary();
    }
    const c = (()=>{ try{return JSON.parse(localStorage.getItem('__cwf_income_cache_v10__')||'null');}catch{return null;} })();
    const allTotal = Number(c?.all_total || 0);

    const res = await fetch(`${API_BASE}/tech/payments_total?v=contract-v10`, { credentials:'include', cache:'no-store' });
    const data = await res.json();
    if (!data || !data.ok) throw new Error(data?.error || 'LOAD_FAILED');
