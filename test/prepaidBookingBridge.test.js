...
test("paid service right opens scheduled step 2 with a valid calendar month", () => {
  ...
  assert.equal(wizardPatches.at(-1)?.step, 2);
  assert.equal(wizardPatches.at(-1)?.error, "");
...
test("redeeming while already on scheduled refreshes the route so availability recovery runs", () => {
  let refreshCount = 0;
  let routeToCount = 0;
  const root = loadBridge({
    state: {
      ...
      setScheduledWizard: () => {},
      setScheduledSubmit: () => {},
      readRouteFromHash: () => "scheduled",
    },
    router: { refresh: () => { refreshCount += 1; } },
    utils: { routeTo: () => { routeToCount += 1; } },
  });

  root.prepaidBookingBridge._test.openScheduledRoute();
  assert.equal(refreshCount, 1);
  assert.equal(routeToCount, 0);
});

test("redeeming from another route navigates to scheduled", () => {
  let refreshCount = 0;
  let route = "";
  const root = loadBridge({
    state: {
      ...
      setScheduledWizard: () => {},
      setScheduledSubmit: () => {},
      readRouteFromHash: () => "store",
    },
    router: { refresh: () => { refreshCount += 1; } },
    utils: { routeTo: (value) => { route = value; } },
  });

  root.prepaidBookingBridge._test.openScheduledRoute();
  assert.equal(refreshCount, 0);
  assert.equal(route, "scheduled");
});
