...
function normalizePhoneLookupClient(phone){
...
async function lookupLatestCustomerDataByPhone(){
  const rawPhone = (el("customer_phone")?.value || "").trim();
  const query = normalizePhoneLookupClient(rawPhone);
  if (query.length < 8) {
    resetCustomerLookupUI();
    return;
  }
  const hint = el("customer_lookup_hint");
  if (hint) hint.textContent = "กำลังค้นหาข้อมูลลูกค้าเก่า...";
  try {
    const r = await apiFetch(`/admin/customer_lookup_by_phone_v2?phone=${encodeURIComponent(rawPhone)}`);
    state.customer_lookup = { query, data: (r && r.found) ? r : null };
    renderCustomerLookupUI(r);
  } catch (e) {
    state.customer_lookup = { query, data: null };
    if (hint) hint.textContent = "ค้นหาข้อมูลลูกค้าเก่าไม่สำเร็จ ลองตรวจสอบเบอร์โทรแล้วค้นหาใหม่";
  }
}

function applyLatestCustomerData(){
...
function updateFlowUI(){
  ...
  const slotsWrap = el('slots_wrap');
  const assignCard = el('assign_card');
  const assignWrap = el('assign_mode_ui_wrap');
  const dateWrap = el('appt_date_wrap');
  const help = el('dispatch_ui_help');
  const appt = el('appointment_datetime');
  const apptLabel = el('appointment_datetime_label');
  const apptHelp = el('appointment_datetime_help');
  ...
  if(uiMode === 'urgent'){
    ...
    if(appt) appt.removeAttribute('readonly');
    if(apptLabel) apptLabel.textContent = 'วันเวลาเร่งด่วน';
    if(apptHelp) apptHelp.textContent = 'ระบบจะยิงข้อเสนอให้ช่างที่เปิดรับงาน + ว่าง + อยู่ในโซนนี้เท่านั้น ใครกดรับก่อนจะได้งาน';
    ...
    if(appt) appt.setAttribute('readonly','readonly');
    if(apptLabel) apptLabel.textContent = 'วันเวลา (ตรวจสอบก่อนบันทึก)';
    if(apptHelp) apptHelp.textContent = '';
  }
...
function syncModesFromUI(){
  ...
  if(assign){
    assign.value = (uiMode === 'urgent') ? 'auto' : ((amUI?.value || 'auto').toString());
  }
...
async function runUrgentPreview(){
  // Removed backend call to non-existing /admin/urgent_preview_v2.
  // Urgent eligibility is validated safely at submit time by /admin/book_v2.
  try{
...
function wireEvents() {
  ...
  el("customer_phone")?.addEventListener("input", () => {
    const next = normalizePhoneLookupClient(el("customer_phone")?.value || "");
    if (!next) {
      resetCustomerLookupUI();
      return;
    }
    if (state.customer_lookup?.query && state.customer_lookup.query !== next) {
