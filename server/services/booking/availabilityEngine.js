...
// Compatibility calculator for the existing forced=1 admin callers of the
// public v2 endpoint. This intentionally preserves the legacy policy until the
// dedicated authorization cutover in a later PR.
async function computeForcedAvailability(deps, options) {
  const {
    pool,
    listTechniciansByType,
    buildOffMapForDate,
  ...
  const travelBufferMin = Number(deps.travelBufferMin ?? jobTiming.TURNAROUND_BUFFER_MIN);
  const date = String(options.date || "");
  const techType = String(options.tech_type || "company");
  // Route parsing is intentionally authoritative here. In particular, legacy
  // invalid numeric input produces NaN-derived output exactly as before.
  const durationMin = Number(options.duration_min);
  const slotStepMin = 30;
  const includeFull = options.include_full === true;
  const mode = String(options.mode || "start");
  ...
  const techsAll = await listTechniciansByType(techType, {
    include_paused: true,
    allow_type_fallback: true,
  });
  const offMap = await buildOffMapForDate(date, techsAll.map((tech) => tech.username));
  const techs = techsAll.filter((tech) => !isTechOffOnDate(tech, date, offMap, { ignoreWeekly: true }));
  ...
  try {
    ...
    for (const row of result.rows || []) {
      const username = row.technician_username;
      if (!specialMap.has(username)) specialMap.set(username, []);
      specialMap.get(username).push({ start: row.start_time, end: row.end_time });
    }
  } catch (error) {
    console.warn("[availability_v2] special slots query failed", error.message);
  }

  const techCount = techs.length;
  if (debugFlag && techCount === 0) {
    debugReasons.push({
      code: "NO_TECH",
      message: "ไม่มีช่างที่ตรงเงื่อนไข (tech_type/วันหยุด/สิทธิ์งาน) — หมายเหตุ: โหมดเปิด/ปิดรับงาน (accept_status) ไม่ถูกนำมาใช้กับการจองของลูกค้า",
    });
  }

  let crewSize = Math.max(1, Math.min(10, Number(options.crew_size || 1) || 1));
  const allowPreviewParallel = options.preview_team === true && String(options.assign_mode || "").toLowerCase() === "team";
  if (!allowPreviewParallel) {
    crewSize = 1;
  } else {
    crewSize = Math.max(1, Math.min(techCount || 1, crewSize));
  }
  ...
  const workEnd = "18:00";
  if (specialMap.size) {
    for (const windows of specialMap.values()) {
      for (const window of windows || []) {
        const start = toMin(window.start);
        const end = toMin(window.end);
        if (Number.isFinite(start) && Number.isFinite(end) && end > start) {
          uiStartMin = Math.min(uiStartMin, start);
          uiEndMin = Math.max(uiEndMin, end);
        }
      }
    }
  }
  ...
  try {
    const now = getNowBangkokParts();
    const today = `${now.Y}-${now.M}-${now.D}`;
    if (mode === "start" && date === today) {
      const nowMin = now.hh * 60 + now.mm;
      const rounded = Math.ceil(nowMin / slotStepMin) * slotStepMin;
      uiStartMin = Math.max(uiStartMin, Math.min(rounded, uiEndMin));
    }
  } catch (_) {
    // Preserve the legacy fail-open behavior for timezone formatting failures.
  }

  const effectiveDurationMin = Math.max(1, Number(durationMin || 0));
  const effectiveBlockMin = effectiveDurationMin + travelBufferMin;
  const events = new Map();
  const addEvent = (minute, type, username) => {
    const key = Math.round(minute);
    if (!events.has(key)) events.set(key, { add: [], remove: [] });
  ...
  for (const tech of techs) {
    const windows = buildTechWindowsMin(tech, date, specialMap, uiStartMin, uiEndMin);
    if (!windows.length) continue;
    const busyBlocks = await listBusyBlocksForTechOnDate(tech.username, date, null);
    for (const window of windows) {
      if (mode === "free") {
        const busy = buildBusyIntervalsConservative(busyBlocks);
        const free = buildFreeIntervalsForWindow(busy, window.startMin, window.endMin);
        if (debugFlag) {
          debugBusy[tech.username] = (debugBusy[tech.username] || []).concat(
            busy.map((block) => ({ start: fmtHHMMFromMin(block.startMin), end: fmtHHMMFromMin(block.endMin) }))
          );
          debugFree[tech.username] = (debugFree[tech.username] || []).concat(
            free.map((block) => ({ start: fmtHHMMFromMin(block.startMin), end: fmtHHMMFromMin(block.endMin) }))
          );
        }
        for (const interval of free) {
          addEvent(interval.startMin, "add", tech.username);
          addEvent(interval.endMin, "remove", tech.username);
        }
      } else {
        const intervals = buildStartIntervalsByCollision(
          busyBlocks,
          window.startMin,
          window.endMin,
  ...
  if (!events.size) {
    if (debugFlag) {
      debugReasons.push({
        code: "NO_EVENTS",
        message: "ไม่มีช่วงเวลาว่าง/ช่วงเริ่มงานในหน้าต่างเวลาที่แสดง (อาจเกิดจากวันหยุด/ไม่มี special slot/หรือถูก busy block ทั้งหมด)",
      });
    }
    console.log("[availability_v2]", {
      ...
      slots: 0,
      reason: debugReasons.map((reason) => reason.code).join(","),
    });
    return {
      date,
      tech_type: techType,
      ...
      effective_block_min: effectiveBlockMin,
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    if (next == null) continue;
    const segmentStart = Math.max(uiStartMin, minute);
    const segmentEndExclusive = Math.min(uiEndMin + 1, next);
    const segmentEnd = mode === "free"
      ? Math.min(uiEndMin, segmentEndExclusive)
      : segmentEndExclusive - 1;
    if (segmentEnd < segmentStart) continue;
    const usernames = Array.from(active);
    const available = usernames.length >= crewSize;
    if (!available && !includeFull) continue;
    if (mode === "start") {
      const lastStart = Math.min(segmentEnd, uiEndMin - slotStepMin);
      for (let start = segmentStart; start <= lastStart; start += slotStepMin) {
        const end = Math.min(uiEndMin, start + slotStepMin);
        slots.push({
          start: minToHHMM(start),
          end: minToHHMM(end),
          available,
          available_tech_ids: available ? usernames : [],
          capacity: techCount,
          available_count: usernames.length,
          crew_size: crewSize,
      ...
      slots.push({
        start: minToHHMM(segmentStart),
        end: minToHHMM(segmentEnd),
        available,
        available_tech_ids: available ? usernames : [],
        capacity: techCount,
        available_count: usernames.length,
  ...
  if (debugFlag && slots.length === 0 && techCount > 0) {
    debugReasons.push({
      code: "BLOCKED",
      message: "พบช่างแต่ไม่มีช่วงที่เริ่มงานได้ (ถูก block จาก busy+buffer หรือ duration ยาวเกินช่วงว่าง)",
    });
  }
  console.log("[availability_v2]", {
    date,
    ...
    tech_count: techCount,
    slots: slots.length,
    reason: debugReasons.length ? debugReasons.map((reason) => reason.code).join(",") : undefined,
  });
  return {
    date,
    tech_type: techType,
...
async function computeAdminAvailabilityByTech(deps, options) {
  const {
    pool,
    listTechniciansByType,
    buildOffMapForDate,
  ...
  const durationMin = Number(options.duration_min);
  const slotStepMin = 30;
  const includePaused = options.include_paused === true;
  const techsAll = await listTechniciansByType(techType, {
    include_paused: includePaused,
    allow_type_fallback: true,
  });
  const offMap = await buildOffMapForDate(date, (techsAll || []).map((tech) => tech.username));
  const techs = (techsAll || []).filter((tech) => (
    !(includePaused && isTechOffOnDate(tech, date, offMap, { ignoreWeekly: true }))
  ));
  ...
  try {
    ...
    console.warn("[admin_availability_by_tech_v2] special slots query failed", error.message);
  }

  let globalStart = toMin("09:00");
  let globalEnd = toMin("18:00");
  for (const tech of techs) {
    const start = toMin(tech.work_start || "09:00");
    const end = toMin(tech.work_end || "18:00");
    if (Number.isFinite(start)) globalStart = Math.min(globalStart, start);
    if (Number.isFinite(end)) globalEnd = Math.max(globalEnd, end);
    for (const window of specialMap.get(tech.username) || []) {
      globalStart = Math.min(globalStart, toMin(window.start));
      globalEnd = Math.max(globalEnd, toMin(window.end));
    }
  }
  globalStart = Math.max(0, Math.min(24 * 60, globalStart));
  globalEnd = Math.max(0, Math.min(24 * 60, globalEnd));
  const workStart = minToHHMM(globalStart);
  const workEnd = minToHHMM(globalEnd);
  const normalizedDurationMin = Math.max(15, Number(durationMin || 60));
  const effectiveBlockMin = normalizedDurationMin + travelBufferMin;
  const allSlots = [];
  for (let minute = globalStart; minute < globalEnd; minute += slotStepMin) {
    let serviceMin = normalizedDurationMin;
    let blockMin = serviceMin + travelBufferMin;
    if (minute + blockMin > globalEnd && minute + serviceMin <= globalEnd) blockMin = serviceMin;
    if (minute + serviceMin > globalEnd && minute + (serviceMin - travelBufferMin) <= globalEnd) {
      serviceMin = Math.max(15, serviceMin - travelBufferMin);
      blockMin = serviceMin;
    }
    if (minute + blockMin > globalEnd) continue;
    allSlots.push({
      start: minToHHMM(minute),
      end: minToHHMM(minute + blockMin),
  ...
  const techRows = [];
  for (const tech of techs) {
    const techStart = toMin(tech.work_start || workStart);
    const techEnd = toMin(tech.work_end || workEnd);
    const specialWindows = specialMap.get(tech.username) || [];
    const slots = [];
    for (const slot of allSlots) {
      const slotStart = toMin(slot.start);
      let within = slotStart >= techStart && slotStart + (slot.block_min || effectiveBlockMin) <= techEnd;
      if (!within) {
        for (const window of specialWindows) {
          const start = toMin(window.start);
          const end = toMin(window.end);
          if (slotStart >= start && slotStart + (slot.block_min || effectiveBlockMin) <= end) {
            within = true;
            break;
          }
        }
      ...
      if (!within) {
        slots.push({ start: slot.start, end: slot.end, available: false });
        continue;
      }
      const free = await isTechFree(
        tech.username,
        `${date}T${slot.start}:00`,
        slot.service_min || durationMin,
        null
      );
      slots.push({ start: slot.start, end: slot.end, available: Boolean(free) });
    }
    techRows.push({ username: tech.username, full_name: tech.full_name || null, slots });
  }

  console.log("[admin_availability_by_tech_v2]", {
    date,
    tech_type: techType,
    duration_min: durationMin,
  ...
  return {
    ...
    slots_by_tech: Object.fromEntries(techRows.map((row) => [row.username, row.slots || []])),
  };
}

async function computeLegacyPublicAvailability(deps, options) {
  const { pool, isTechFree } = deps;
  const date = String(options.date || "");
  const start = String(options.start || "08:00");
  const end = String(options.end || "18:00");
  const slotMin = Number(options.slot_min);
  const technicianResult = await pool.query(`
    ...
    `);
  const technicians = (technicianResult.rows || [])
  ...
  const starts = [];
  for (let minute = startMin; minute + slotMin <= endMin; minute += slotMin) starts.push(minute);
  const slots = [];
  for (const minute of starts) {
    const hour = String(Math.floor(minute / 60)).padStart(2, "0");
    const minutePart = String(minute % 60).padStart(2, "0");
    const iso = `${date}T${hour}:${minutePart}:00+07:00`;
    let freeCount = 0;
    for (const technician of technicians) {
      if (await isTechFree(technician, iso, slotMin, null)) freeCount += 1;
    }
    slots.push({
      time: `${hour}:${minutePart}`,
      available: technicians.length === 0 ? false : freeCount > 0,
      capacity: technicians.length,
      busy: Math.max(0, technicians.length - freeCount),
    });
  }
  return {
    date,
...
module.exports = {
  ...
  computeForcedAvailability,
  computeAdminAvailabilityByTech,
