...
VERSION = "ai-office-practical-admin-ux-v13-20260607"
...
def patch_ai_office_js():
  ...
  function draftInstructionForTool(tool, label){
    const base = "ใช้เฉพาะแชทลูกค้าคนนี้ ร่างข้อความแบบแอดมิน LINE ของ Coldwindflow ให้สั้น สุภาพ ธรรมชาติ พร้อมคัดลอกส่งลูกค้า ห้ามใส่สรุปหรือหมายเหตุใน customer_reply";
    const map = {
...
'''
    ...
    if strict in text and "customer_reply must sound like a real Coldwindflow LINE admin" not in text:
        add = '''
    "customer_reply must sound like a real Coldwindflow LINE admin, not a report: short, natural, polite, usually 1-4 lines.",
    ...
    "Examples of good Thai LINE style: สวัสดีครับ รบกวนแจ้งพื้นที่ให้บริการ จำนวนเครื่อง และวันเวลาที่สะดวกเพิ่มเติมได้ไหมครับ เดี๋ยวแอดมินตรวจสอบคิวและแจ้งรายละเอียดให้นะครับ 🙏",'''
...
function fallbackLineDraft({ conversation, messages, threadContext }) {
  ...
  if (["en", "ja", "zh", "ko"].includes(language)) {
    ...
    reply = "สวัสดีครับ ราคาล้างแอร์ผนังเริ่มต้น 550 บาทครับ รบกวนแจ้งขนาด BTU / จำนวนเครื่อง และพื้นที่ให้บริการเพิ่มเติมได้ไหมครับ เดี๋ยวแอดมินเช็กคิวให้ครับ 🙏";
  } else if (looksLikeExpensive) {
    reply = "เข้าใจครับ งานของ Coldwindflow จะมีการตรวจเช็กเบื้องต้น ล้างคอยล์เย็น/คอยล์ร้อน และรับประกันงานล้าง 30 วันครับ ถ้าลูกค้าสะดวก ผมช่วยดูแพ็กเกจที่เหมาะกับอาการและงบให้ได้ครับ";
  } else if (hasPartialPhone) {
    reply = "สวัสดีครับ รบกวนส่งเบอร์โทรติดต่อกลับแบบครบถ้วน พร้อมพื้นที่ให้บริการและจำนวนเครื่องเพิ่มเติมได้ไหมครับ เดี๋ยวแอดมินตรวจสอบคิวและแจ้งรายละเอียดให้นะครับ 🙏";
  } else {
    reply = "สวัสดีครับ รบกวนแจ้งพื้นที่ให้บริการ จำนวนเครื่อง และวันเวลาที่สะดวกเพิ่มเติมได้ไหมครับ เดี๋ยวแอดมินตรวจสอบคิวและแจ้งรายละเอียดให้นะครับ 🙏";
  }
  return {
...
def main():
    ...
    patch_backend_route()
    print("CWF AI Office Practical Admin UX v13 applied.")
    print("Changed files:")
    for rel in CHANGED:
        print("-", rel)
