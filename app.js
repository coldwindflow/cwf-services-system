...
let __CWF_OFFER_COUNTDOWN_TIMER__ = null;
function updateOfferCountdownBadges(){
  try {
    document.querySelectorAll('[data-offer-expires]').forEach((el)=>{
      const expires = Number(el.getAttribute('data-offer-expires') || 0);
      const secLeft = Math.max(0, Math.floor((expires - Date.now()) / 1000));
      const min = Math.floor(secLeft / 60);
      const sec = secLeft % 60;
      el.textContent = `⏳ ${min}:${String(sec).padStart(2, '0')}`;
      if (secLeft <= 0) el.textContent = '⏳ หมดเวลา';
    });
  } catch(_) {}
}
function startOfferCountdownTicker(){
  updateOfferCountdownBadges();
  if (__CWF_OFFER_COUNTDOWN_TIMER__) clearInterval(__CWF_OFFER_COUNTDOWN_TIMER__);
  __CWF_OFFER_COUNTDOWN_TIMER__ = setInterval(updateOfferCountdownBadges, 1000);
}
...
function ensureCwfCloseStyles(){
  ...
  style.textContent = `
    ...
    .cwf-unit-toolbar{display:grid;grid-template-columns:1fr;gap:10px;margin-bottom:12px}.cwf-unit-select{width:100%;border:1px solid rgba(37,99,235,.18);border-radius:16px;padding:12px;background:#fff;font-weight:900;color:#0f172a}.cwf-unit-tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px}.cwf-unit-tab{border:1px solid rgba(37,99,235,.18)!important;background:#fff!important;color:#0f172a!important;border-radius:16px!important;padding:12px 10px!important;font-weight:1000!important}.cwf-unit-tab.active{background:linear-gradient(135deg,#1558d6,#05b6d6)!important;color:#fff!important;border-color:transparent!important}.cwf-unit-summary{background:#fff;border:1px solid rgba(37,99,235,.14);border-radius:18px;padding:12px;margin-bottom:12px;box-shadow:0 10px 24px rgba(15,23,42,.05)}.cwf-unit-summary b{font-size:17px}.cwf-unit-owner{display:inline-flex;align-items:center;gap:6px;border-radius:999px;background:#ecfdf5;color:#047857;border:1px solid #a7f3d0;padding:5px 9px;font-size:12px;font-weight:1000}.cwf-photo-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.cwf-photo-card{background:#fff;border:1px solid rgba(37,99,235,.15);border-radius:18px;padding:12px;box-shadow:0 10px 26px rgba(15,23,42,.06)}.cwf-photo-card b{font-size:16px}.cwf-photo-card .muted{font-size:12px}.cwf-photo-card button{width:100%;margin-top:9px;border-radius:15px}.cwf-thumb-row{display:flex;gap:6px;overflow:auto;margin-top:8px}.cwf-thumb-row img{width:54px;height:54px;border-radius:12px;object-fit:cover;border:1px solid rgba(15,23,42,.12);background:#fff}.cwf-thumb{min-width:64px;max-width:90px}.cwf-thumb-note{font-size:11px;color:#475569;margin-top:3px;line-height:1.25}.cwf-photo-note{width:100%;border:1px solid rgba(15,23,42,.14);border-radius:12px;padding:8px 10px;margin-top:8px;font-size:13px;background:#f8fafc}.cwf-check-select{width:100%;border:1px solid rgba(37,99,235,.18);border-radius:14px;padding:11px 12px;font-weight:900;background:#fff;margin:8px 0 10px}
...
function cwfTechToast(message, isError = false) {
  try {
    const old = document.getElementById('cwfTechToast');
    if (old) old.remove();
    const el = document.createElement('div');
    el.id = 'cwfTechToast';
    el.textContent = String(message || '');
    el.style.cssText = `position:fixed;left:50%;bottom:calc(22px + env(safe-area-inset-bottom));transform:translateX(-50%);z-index:120001;max-width:min(420px,92vw);padding:13px 16px;border-radius:999px;font-weight:1000;font-size:14px;line-height:1.35;box-shadow:0 16px 36px rgba(2,6,23,.22);background:${isError ? '#fee2e2' : '#dcfce7'};color:${isError ? '#991b1b' : '#14532d'};border:1px solid ${isError ? '#fecaca' : '#86efac'};`;
    ...
    for (const f of selected) {
      ...
      const metaRes = await fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/photos/meta`, {
        ...
        body: JSON.stringify({
          ...
          photo_note: String(photoNote || '').slice(0, 500)
        })
      });
...
function cwfUnitShortDetail(u){
  const parts = [];
  const item = String(u?.item_name || '').trim().replace(/\s*(?:x|×)\s*\d+\s*$/i,'').replace(/\s*\d+\s*เครื่อง\s*$/i,'').trim();
  if (item) parts.push(item);
  if (u?.ac_type && !parts.join(' ').includes(String(u.ac_type))) parts.push(String(u.ac_type));
  if (u?.wash_type && String(parts.join(' ')).includes('ผนัง')) parts.push(String(u.wash_type));
  if (u?.btu) parts.push(`${u.btu} BTU`);
  return parts.filter(Boolean).join(' • ') || 'เครื่องปรับอากาศ';
}
function cwfUnitOptionLabel(u){
  const owner = u?.assigned_technician ? ` • ช่าง: ${u.assigned_technician}` : '';
  return `${cwfUnitTitle(u)} • ${cwfUnitShortDetail(u)}${owner}`;
}


function cwfPickUnitPhoto(jobId, unitId, phase){
  // สำคัญมากบนมือถือ/WebView: ต้องเปิด file picker ทันทีใน user gesture
  // ห้าม await โหลดข้อมูลก่อน input.click() ไม่งั้นบางเครื่องจะกดแล้วไม่เกิดอะไรขึ้น
...
async function cwfAttachChecklistIssuePhoto(checkKey, section, idx, label){
  try {
    const ctx = window.__cwfChecklistContext || null;
    if (!ctx || ctx.mode !== 'unit' || !ctx.jobId || !ctx.unitId) {
      return alert('กรุณาเปิดเช็คลิสจากหน้าหลักฐานแยกเครื่องก่อนแนบรูป');
    }
    const note = `รูปประกอบเช็คลิส${section === 'pre' ? 'ก่อนทำ' : 'หลังทำ'} ข้อ ${Number(idx)+1}: ${label || ''}`.slice(0,500);
    await cwfPickUnitPhotoWithNote(ctx.jobId, ctx.unitId, 'defect', note);
  } catch(e) { alert(e.message || 'แนบรูปปัญหาไม่สำเร็จ'); }
}
window.cwfAttachChecklistIssuePhoto = cwfAttachChecklistIssuePhoto;

function cwfPickUnitPhotoWithNote(jobId, unitId, phase, photoNote){
  // สำคัญ: เปิด file picker ทันทีจากปุ่ม ไม่ await ก่อน เพื่อให้มือถือยอมเปิดกล้อง/แกลเลอรี
...
window.cwfPickUnitPhotoWithNote = cwfPickUnitPhotoWithNote;
...
async function cwfSaveUnitChecklist(jobId, unitId, section){
  ...
  let d = {};
  try {
    d = await cwfSaveUnitChecklistRows(jobId, unitId, section, rows);
  } catch(e) {
    return alert(e.message || 'บันทึกเช็คลิสเครื่องนี้ไม่สำเร็จ');
  }
  alert(d.message || 'บันทึกเช็คลิสเครื่องนี้แล้ว');
  openTechPhotoModal(jobId, 'checklist', unitId);
}
window.cwfSaveUnitChecklist = cwfSaveUnitChecklist;

function cwfBuildNormalChecklistRows(section){
  const items = section === 'pre' ? CWF_PRE_CHECK_ITEMS : CWF_POST_CHECK_ITEMS;
  return items.map(label => ({ label, checked:true, issue:false, note:'', status:'ปกติ', auto_normal:true }));
}

async function cwfSaveUnitChecklistRows(jobId, unitId, section, rows){
  const r = await fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/units/${encodeURIComponent(String(unitId))}/checklist`, {
    method:'PUT',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ checklist_type:section, checklist_json:rows, technician_username:(typeof username === 'string' ? username : '') || null })
  });
  const d = await r.json().catch(()=>({}));
  if (!r.ok) throw new Error(d.error || 'บันทึกเช็คลิสเครื่องนี้ไม่สำเร็จ');
  return d;
}

function cwfAskAutoNormalChecklistAck(missingLabels){
  return new Promise((resolve)=>{
    const list = (missingLabels || []).slice(0, 12).map(x=>`<li>${escapeHTML(x)}</li>`).join('');
    const body = `<div class="cwf-pay-card"><b style="font-size:18px;color:#be123c">ยืนยันเช็คลิสเป็นปกติอัตโนมัติ</b><p>ยังมีเช็คลิสบางรายการที่ช่างไม่ได้กดบันทึก ระบบจะบันทึกเป็น <b>ปกติทั้งหมด</b> ทั้งก่อนทำ/หลังทำสำหรับเครื่องที่ยังขาด</p><ul style="margin:8px 0 10px 20px">${list}</ul><p>หากภายหลังลูกค้าร้องเรียนหรือมีปัญหาที่เกี่ยวข้องกับรายการตรวจเช็ค ช่างผู้ปิดงานต้องรับผิดชอบตามข้อเท็จจริงและเงื่อนไขการรับประกันงาน ไม่มีข้ออ้างว่าไม่ได้ตรวจหรือไม่ได้ระบุไว้</p><label style="display:flex;gap:10px;align-items:flex-start;font-weight:900"><input id="cwfAutoChecklistAck" type="checkbox" style="width:24px;height:24px"> ข้าพเจ้าเข้าใจและยอมรับเงื่อนไขนี้</label></div>`;
    const footer = `<button class="secondary" type="button" onclick="cwfCloseModal(); window.__cwfAutoChecklistResolve && window.__cwfAutoChecklistResolve(false)">กลับไปทำเช็คลิส</button><button type="button" onclick="if(!document.getElementById('cwfAutoChecklistAck')?.checked){alert('กรุณาติ๊กยอมรับเงื่อนไขก่อน');return;} cwfCloseModal(); window.__cwfAutoChecklistResolve && window.__cwfAutoChecklistResolve(true)">ยืนยันและบันทึกเป็นปกติ</button>`;
    window.__cwfAutoChecklistResolve = resolve;
    cwfOpenModal('ยืนยันเช็คลิสก่อนปิดงาน', body, footer);
  });
}

async function cwfAutoCompleteMissingUnitChecklists(jobId, units){
  const missing = [];
  for (const u of units || []) {
    const label = `เครื่องที่ ${u.unit_no || '-'} รหัส ${u.unit_code || '-'}`;
    if (!(u.checklist && u.checklist.pre && u.checklist.pre.completed)) missing.push({ unit:u, section:'pre', label:`${label} เช็คลิสก่อนทำ` });
    if (!(u.checklist && u.checklist.post && u.checklist.post.completed)) missing.push({ unit:u, section:'post', label:`${label} เช็คลิสหลังทำ` });
  }
  if (!missing.length) return true;
  const ok = await cwfAskAutoNormalChecklistAck(missing.map(x=>x.label));
  if (!ok) return false;
  for (const m of missing) await cwfSaveUnitChecklistRows(jobId, m.unit.unit_id, m.section, cwfBuildNormalChecklistRows(m.section));
  return true;
}



async function cwfLoadUnitPhotos(jobId, unitId){
  try {
    const r = await fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/photos?unit_id=${encodeURIComponent(String(unitId))}`, { cache:'no-store' });
    const d = await r.json().catch(()=>[]);
    return r.ok && Array.isArray(d) ? d.filter(x => x && x.public_url) : [];
  } catch { return []; }
}

function cwfRenderUnitEvidenceModal(jobId, units, tab, selectedUnitId, unitPhotos){
  const current = (typeof username === 'string' ? username : '') || '';
  const selected = units.find(u => String(u.unit_id) === String(selectedUnitId)) || units[0];
  const sid = selected?.unit_id;
  const active = tab === 'checklist' ? 'checklist' : 'photos';
  const counts = selected?.photo_counts || {};
  const preOk = !!(selected?.checklist && selected.checklist.pre && selected.checklist.pre.completed);
  const postOk = !!(selected?.checklist && selected.checklist.post && selected.checklist.post.completed);
  const mine = current && String(selected?.assigned_technician || '') === current;
  const photos = Array.isArray(unitPhotos) ? unitPhotos : [];
  const photoByPhase = (phase) => photos.filter(p => String(p.phase || '') === phase).slice(0, 8);
  const jobSafe = String(jobId).replace(/'/g,"\\'");
  const optionHtml = units.map(u => `<option value="${String(u.unit_id)}" ${String(u.unit_id)===String(sid)?'selected':''}>${escapeHTML(cwfUnitOptionLabel(u))}</option>`).join('');
  const ownerHtml = selected?.assigned_technician ? `<span class="cwf-unit-owner">👷 ผู้รับผิดชอบ: ${escapeHTML(selected.assigned_technician)}</span>` : '';
  const summary = `
    <div class="cwf-unit-toolbar">
      <select class="cwf-unit-select" onchange="openTechPhotoModal('${jobSafe}', '${active}', this.value)">${optionHtml}</select>
      <div class="cwf-unit-tabs">
        <button type="button" class="cwf-unit-tab ${active==='photos'?'active':''}" onclick="openTechPhotoModal('${jobSafe}', 'photos', '${sid}')">📷 รูปภาพ</button>
        <button type="button" class="cwf-unit-tab ${active==='checklist'?'active':''}" onclick="openTechPhotoModal('${jobSafe}', 'checklist', '${sid}')">✅ เช็คลิส</button>
      </div>
    </div>
    <div class="cwf-unit-summary" style="${mine ? 'border-color:#1558d6;box-shadow:0 0 0 2px rgba(21,88,214,.10)' : ''}">
      <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:flex-start">
        <div><b>${escapeHTML(cwfUnitTitle(selected))}</b><div class="muted" style="margin-top:4px">${escapeHTML(cwfUnitShortDetail(selected))}</div></div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">${mine ? '<span class="cwf-chip ok">เครื่องของฉัน</span>' : ''}${ownerHtml}</div>
      </div>
    </div>`;
  if (active === 'checklist') {
    const selectedSection = window.__cwfUnitChecklistSection || 'pre';
    window.__cwfChecklistContext = { mode:'unit', jobId:String(jobId), unitId:String(sid), section:selectedSection };
    const sectionOk = selectedSection === 'pre' ? preOk : postOk;
    const sectionKey = `${cwfCloseKey(jobId)}_unit_${sid}`;
    const statusText = selectedSection === 'pre' ? 'เช็คลิสก่อนทำ' : 'เช็คลิสหลังทำ';
    return summary + `
      <div class="cwf-photo-card">
        <label style="font-weight:1000;color:#0b2e6d">เลือกหน้าเช็คลิส</label>
        <select class="cwf-check-select" onchange="window.__cwfUnitChecklistSection=this.value; openTechPhotoModal('${jobSafe}', 'checklist', '${sid}')">
          <option value="pre" ${selectedSection==='pre'?'selected':''}>🔎 เช็คลิสก่อนทำ</option>
          <option value="post" ${selectedSection==='post'?'selected':''}>✅ เช็คลิสหลังทำ</option>
        </select>
        <div class="cwf-mini-status"><span class="cwf-chip ${sectionOk ? 'ok':'bad'}">${statusText}: ${sectionOk ? 'บันทึกแล้ว' : 'ยังไม่ครบ'}</span></div>
        ${cwfChecklistSectionHtml(sectionKey, selectedSection)}
        <button type="button" style="margin-top:12px" onclick="cwfSaveUnitChecklist('${jobSafe}', '${sid}', '${selectedSection}')">บันทึก${statusText}</button>
        <div class="muted" style="margin-top:8px;font-size:12px">ถ้าไม่ทำเช็คลิสก่อนปิดงาน ระบบจะให้ยืนยันบันทึกเป็น “ปกติ” ทั้งหมด และช่างต้องรับผิดชอบตามเงื่อนไขรับประกันหากมีข้อร้องเรียน</div>
      </div>`;
  }
  const phaseDefs = [
    ['nameplate','🏷️ เนมเพลท','บังคับใส่ เพื่อยืนยันเครื่องที่ทำ'],
    ['before','📷 รูปก่อนทำ','บังคับใส่ ก่อนเริ่มงาน'],
    ['after','📷 รูปหลังทำ','บังคับใส่ หลังทำเสร็จ'],
    ['pressure','🧪 วัดน้ำยา','ไม่บังคับ'],
    ['current','⚡ วัดกระแส','ไม่บังคับ'],
    ['defect','⚠️ ตำหนิ','ไม่บังคับ'],
  ];
...
async function openTechPhotoModal(jobId, tab = 'photos', selectedUnitId = null){
  const key = cwfCloseKey(jobId);
  ...
  if (units.length && !revisitFlow) {
    ...
    const photos = selected ? await cwfLoadUnitPhotos(key, selected) : [];
    const html = cwfRenderUnitEvidenceModal(key, units, tab, selected, photos);
    cwfOpenModal('หลักฐานแยกตามเครื่อง', html, `<button type="button" class="secondary" onclick="cwfCloseModal()">ปิด</button>`);
    return;
  }
...
// =======================================
// 🔁 REVISIT FLOW — New functions (appointment, media hub, photo, checklist)
// =======================================

// --- Storage helpers for revisit checklist data (localStorage) ---
function cwfRevisitChecklistKey(jobId){ return 'cwf_revisit_cl_' + String(jobId); }
function cwfGetRevisitChecklist(jobId){
  try { return JSON.parse(localStorage.getItem(cwfRevisitChecklistKey(jobId)) || 'null') || {}; }
  catch { return {}; }
}
function cwfSaveRevisitChecklist(jobId, data){
  try { localStorage.setItem(cwfRevisitChecklistKey(jobId), JSON.stringify(data || {})); } catch {}
}

// --- Storage helpers for revisit appointment ---
function cwfRevisitApptKey(jobId){ return 'cwf_revisit_appt_' + String(jobId); }
function cwfGetRevisitAppt(jobId){
  try { return JSON.parse(localStorage.getItem(cwfRevisitApptKey(jobId)) || 'null') || null; }
  catch { return null; }
}
function cwfSaveRevisitAppt(jobId, isoStr){
  try { localStorage.setItem(cwfRevisitApptKey(jobId), JSON.stringify(isoStr)); } catch {}
}

// --- Check revisit close requirements: before+after photos, cause, outcome ---
async function checkRevisitCloseReady(jobId) {
  const cl = cwfGetRevisitChecklist(jobId);
  const cause = String(cl.cause || '').trim();
  const outcome = String(cl.outcome || '').trim();
  if (!cause) return { ok: false, msg: 'งานแก้ไขยังปิดงานไม่ได้: ต้องเลือก "สาเหตุงานแก้ไข" ก่อนกด เสร็จสิ้น' };
  if (!outcome) return { ok: false, msg: 'งานแก้ไขยังปิดงานไม่ได้: ต้องเลือก "ผลการแก้ไข" ก่อนกด เสร็จสิ้น' };
  // check photos
  const localPhotos = await idbGetByJob(jobId).catch(() => []);
  const hasBefore = (localPhotos || []).some(x => x?.phase === 'revisit_before');
  if (!hasBefore) {
    try {
      const rr = await fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/photos`);
      if (rr.ok) {
        const ups = await rr.json().catch(() => []);
        if (!(ups || []).some(x => x?.phase === 'revisit_before' && x?.public_url)) {
          return { ok: false, msg: 'งานแก้ไขยังปิดงานไม่ได้: ต้องมีรูปก่อนแก้ไข (revisit_before) อย่างน้อย 1 รูป' };
        }
      } else {
        return { ok: false, msg: 'งานแก้ไขยังปิดงานไม่ได้: ต้องมีรูปก่อนแก้ไข (revisit_before) อย่างน้อย 1 รูป' };
      }
  ...
  const hasAfter = (localPhotos || []).some(x => x?.phase === 'revisit_after');
  if (!hasAfter) {
    try {
      const rr = await fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/photos`);
      if (rr.ok) {
        const ups = await rr.json().catch(() => []);
        if (!(ups || []).some(x => x?.phase === 'revisit_after' && x?.public_url)) {
          return { ok: false, msg: 'งานแก้ไขยังปิดงานไม่ได้: ต้องมีรูปหลังแก้ไข (revisit_after) อย่างน้อย 1 รูป' };
        }
      } else {
        return { ok: false, msg: 'งานแก้ไขยังปิดงานไม่ได้: ต้องมีรูปหลังแก้ไข (revisit_after) อย่างน้อย 1 รูป' };
      }
...
// ---- 1) แจ้งเวลานัดหมาย ----
async function openRevisitAppointmentModal(jobId) {
  const saved = cwfGetRevisitAppt(jobId);
  const savedVal = saved ? new Date(saved).toISOString().slice(0,16) : '';

  // Load existing jobs for conflict check hint
  const appt = cwfGetCachedJob(cwfCloseKey(jobId))?.appointment_datetime;
  const apptTxt = appt ? new Date(appt).toLocaleString('th-TH') : '-';

  const bodyHtml = `
    <div style="margin-bottom:12px;">
      <div style="font-size:13px;color:#475569;margin-bottom:8px;">นัดหมายปัจจุบันของงานนี้: <b>${escapeHTML(apptTxt)}</b></div>
      <label style="font-weight:900;font-size:14px;">วันและเวลาที่ตกลงกับลูกค้า</label>
      <input type="datetime-local" id="revisitApptInput" style="width:100%;margin-top:8px;border-radius:14px;border:1px solid rgba(37,99,235,.2);padding:10px 12px;font-size:15px;" value="${escapeHTML(savedVal)}">
    </div>
    <div id="revisitApptStatus" style="margin-top:8px;font-size:13px;"></div>`;

  const footerHtml = `
    <button class="secondary" type="button" onclick="cwfCloseModal()">ยกเลิก</button>
    <button type="button" onclick="saveRevisitAppointment('${String(jobId).replace(/'/g,"\\'")}')">💾 บันทึกเวลานัดหมาย</button>`;

  cwfOpenModal('🕛 แจ้งเวลานัดหมาย', bodyHtml, footerHtml);
}
window.openRevisitAppointmentModal = openRevisitAppointmentModal;

async function saveRevisitAppointment(jobId) {
  const el = document.getElementById('revisitApptInput');
  const statusEl = document.getElementById('revisitApptStatus');
  const val = el ? el.value.trim() : '';
  if (!val) { if (statusEl) statusEl.textContent = '⚠️ กรุณาเลือกวันและเวลาก่อน'; return; }

  const chosen = new Date(val);
  if (isNaN(chosen.getTime())) { if (statusEl) statusEl.textContent = '⚠️ วันเวลาไม่ถูกต้อง'; return; }

  // Check conflict: load all active jobs and see if time overlaps
  let hasConflict = false;
  try {
    const r = await fetch(`${API_BASE}/jobs/my`);
    if (r.ok) {
      const jobs = await r.json().catch(() => []);
      for (const j of (Array.isArray(jobs) ? jobs : [])) {
        if (String(j.job_id || j.id || '') === String(jobId)) continue;
        const jAppt = j.appointment_datetime ? new Date(j.appointment_datetime) : null;
        if (!jAppt || isNaN(jAppt.getTime())) continue;
        const diffMin = Math.abs(chosen - jAppt) / 60000;
        if (diffMin < 90) { hasConflict = true; break; }
      }
    }
  } catch {}

  if (hasConflict) {
    cwfOpenModal('⚠️ เวลาชนกับงานอื่น',
      `<div style="line-height:1.7;font-size:14px;">
        <b>เวลานี้ชนกับงานอื่นที่มีอยู่แล้ว</b>
        <div style="margin-top:10px;">หากจำเป็นต้องเข้าช่วงเวลานี้จริง ๆ<br>กรุณาติดต่อแอดมินเพื่อย้ายคิวงานเดิมก่อน<br>หรือเลือกเวลานัดหมายใหม่</div>
      </div>`,
      `<button type="button" onclick="cwfCloseModal();openRevisitAppointmentModal('${String(jobId).replace(/'/g,"\\'")}')">เลือกเวลาใหม่</button>`
    );
    return;
  }

  // Save locally
  cwfSaveRevisitAppt(jobId, chosen.toISOString());

  // Try save to server (non-blocking)
  fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/revisit-appointment`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ revisit_appointment_at: chosen.toISOString() }),
  }).catch(() => {});

  cwfCloseModal();
  alert('✅ บันทึกเวลานัดหมายแล้ว: ' + chosen.toLocaleString('th-TH'));
}
window.saveRevisitAppointment = saveRevisitAppointment;

// ---- 2) Media Hub (เลือก รูป / เช็คลิส) ----
function openRevisitMediaHub(jobId) {
  const bodyHtml = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:4px;">
      <button type="button" onclick="cwfCloseModal();openRevisitPhotoModal('${String(jobId).replace(/'/g,"\\'")}');"
        style="border-radius:18px;min-height:90px;background:#eff6ff;border:1.5px solid rgba(37,99,235,0.22);color:#1558d6;font-weight:900;font-size:14px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;">
        <span style="font-size:28px;">📷</span>
        <span>ลงรูปงานแก้ไข</span>
      </button>
      <button type="button" onclick="cwfCloseModal();openRevisitChecklistModal('${String(jobId).replace(/'/g,"\\'")}');"
        style="border-radius:18px;min-height:90px;background:#f0fdf4;border:1.5px solid rgba(22,163,74,0.22);color:#166534;font-weight:900;font-size:14px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;">
        <span style="font-size:28px;">✅</span>
        <span>เช็คลิสงานแก้ไข</span>
      </button>
    </div>`;
  cwfOpenModal('📷✅ รูปและเช็คลิสงานแก้ไข', bodyHtml, `<button class="secondary" type="button" onclick="cwfCloseModal()">ปิด</button>`);
}
window.openRevisitMediaHub = openRevisitMediaHub;

// ---- 3) ลงรูปงานแก้ไข ----
async function openRevisitPhotoModal(jobId) {
  const key = cwfCloseKey(jobId);
  const canEdit = (() => {
    const j = cwfGetCachedJob(key);
    return !j || !isDoneStatusValue(normStatus(j.job_status));
  })();
  cwfOpenModal('📷 ลงรูปงานแก้ไข', '<div class="muted">กำลังโหลด...</div>');

  const { counts, urls } = await cwfCountPhotos(key);

  const revisitDefs = [
    ['revisit_before', '🔴 รูปก่อนแก้ไข', true],
    ['revisit_after',  '✅ รูปหลังแก้ไข',  true],
    ['revisit_defect', '⚠️ รูปจุดปัญหา',   false],
  ];

  const html = `
    <div class="cwf-mini-status" style="margin-bottom:10px;">
      <span class="cwf-chip ${counts.revisit_before ? 'ok':'bad'}">ก่อนแก้ ${counts.revisit_before || 0} รูป${counts.revisit_before ? '' : ' (บังคับ)'}</span>
      <span class="cwf-chip ${counts.revisit_after ? 'ok':'bad'}">หลังแก้ ${counts.revisit_after || 0} รูป${counts.revisit_after ? '' : ' (บังคับ)'}</span>
    </div>
    <div class="cwf-photo-grid">
      ${revisitDefs.map(([phase, label, required]) => `
        <div class="cwf-photo-card" id="revisit-photo-card-${phase}">
          <div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;">
            <b>${escapeHTML(label)}</b>
            <span class="cwf-chip ${counts[phase] ? 'ok' : (required ? 'bad' : '')}">${counts[phase] ? counts[phase] + ' รูป' : (required ? 'บังคับ' : 'ถ้ามี')}</span>
          </div>
          <div class="muted" style="margin-top:5px;">${counts[phase] ? 'มีแล้ว ' + counts[phase] + ' รูป' : 'ยังไม่มีรูป'}</div>
          ${(urls[phase] || []).length ? '<div class="cwf-thumb-row">' + urls[phase].slice(0,6).map(u => '<a href="' + u + '" target="_blank"><img src="' + u + '" alt="' + escapeHTML(label) + '"></a>').join('') + '</div>' : ''}
          <button type="button" ${canEdit ? '' : 'disabled'} onclick="cwfPickRevisitPhotoAndRefresh('${String(key).replace(/'/g,"\\'")}', '${phase}')">📷 เพิ่มรูป</button>
          <div id="cwfRevisitPhotoUploadStatus_${phase}" class="cwf-inline-upload"></div>
        </div>`).join('')}
    </div>
    <div class="cwf-mini-status" style="margin-top:12px;">
      <button class="secondary" type="button" style="width:auto;" onclick="openUploadedPhotos('${String(key).replace(/'/g,"\\'")}')">🖼️ ดูรูปที่อัปโหลดแล้ว</button>
      <button class="secondary" type="button" style="width:auto;" onclick="forceUpload('${String(key).replace(/'/g,"\\'")}')">⬆️ อัปโหลดค้างในเครื่อง</button>
    </div>`;

  cwfOpenModal('📷 ลงรูปงานแก้ไข', html, `<button class="secondary" type="button" onclick="cwfCloseModal()">ปิด</button>`);
}
window.openRevisitPhotoModal = openRevisitPhotoModal;

function cwfPickRevisitPhotoAndRefresh(jobId, phase) {
  try {
    pickPhotos(jobId, phase, 12);
    setTimeout(() => openRevisitPhotoModal(jobId), 900);
  } catch(e) { alert(e.message || 'เลือกรูปไม่สำเร็จ'); }
}
window.cwfPickRevisitPhotoAndRefresh = cwfPickRevisitPhotoAndRefresh;

// ---- 4) เช็คลิสงานแก้ไข ----
function openRevisitChecklistModal(jobId) {
  const cl = cwfGetRevisitChecklist(jobId);
  const cause = escapeHTML(cl.cause || '');
  const causeDesc = escapeHTML(cl.cause_desc || '');
  const outcome = escapeHTML(cl.outcome || '');
  const outcomeDesc = escapeHTML(cl.outcome_desc || '');

  const bodyHtml = `
    <div style="display:flex;flex-direction:column;gap:14px;">
      <div>
        <label style="font-weight:900;">1. สาเหตุงานแก้ไข <span style="color:#dc2626;">*</span></label>
        <select id="revisitCauseSelect" style="width:100%;margin-top:6px;border-radius:14px;border:1px solid rgba(37,99,235,.2);padding:10px 12px;">
          <option value="">— กรุณาเลือก —</option>
          <option value="tech_fault" ${cause==='tech_fault'?'selected':''}>เกิดจากช่าง / งานเดิม</option>
          <option value="customer_usage" ${cause==='customer_usage'?'selected':''}>เกิดจากการใช้งานของลูกค้า</option>
          <option value="system_part" ${cause==='system_part'?'selected':''}>เกิดจากระบบ / อะไหล่ / เงื่อนไขบริษัท</option>
          <option value="unclear" ${cause==='unclear'?'selected':''}>ยังไม่ชัดเจน ให้แอดมินตรวจ</option>
        </select>
      </div>
      <div>
        <label style="font-weight:900;">2. คำอธิบายสาเหตุที่พบ <span style="color:#94a3b8;font-weight:400;">(ถ้ามี)</span></label>
        <textarea id="revisitCauseDesc" rows="2" style="width:100%;margin-top:6px;border-radius:14px;border:1px solid rgba(37,99,235,.2);padding:10px 12px;" placeholder="อธิบายเพิ่มเติม...">${causeDesc}</textarea>
      </div>
      <div>
        <label style="font-weight:900;">3. ผลการแก้ไข <span style="color:#dc2626;">*</span></label>
        <select id="revisitOutcomeSelect" style="width:100%;margin-top:6px;border-radius:14px;border:1px solid rgba(37,99,235,.2);padding:10px 12px;">
          <option value="">— กรุณาเลือก —</option>
          <option value="successful" ${outcome==='successful'?'selected':''}>แก้ไขสำเร็จ ใช้งานได้ปกติ</option>
          <option value="pending_admin" ${outcome==='pending_admin'?'selected':''}>ยังไม่จบ / ยังมีอาการ ต้องให้แอดมินติดตาม</option>
        </select>
      </div>
      <div>
        <label style="font-weight:900;">4. คำอธิบายผลการแก้ไข <span style="color:#94a3b8;font-weight:400;">(ถ้ามี)</span></label>
        <textarea id="revisitOutcomeDesc" rows="2" style="width:100%;margin-top:6px;border-radius:14px;border:1px solid rgba(37,99,235,.2);padding:10px 12px;" placeholder="อธิบายผลเพิ่มเติม...">${outcomeDesc}</textarea>
      </div>
      <div id="revisitChecklistStatus" style="font-size:13px;"></div>
    </div>`;

  const footerHtml = `
    <button class="secondary" type="button" onclick="cwfCloseModal()">ยกเลิก</button>
    <button type="button" onclick="saveRevisitChecklist('${String(jobId).replace(/'/g,"\\'")}')">💾 บันทึกเช็คลิส</button>`;

  cwfOpenModal('✅ เช็คลิสงานแก้ไข', bodyHtml, footerHtml);
}
window.openRevisitChecklistModal = openRevisitChecklistModal;

function saveRevisitChecklist(jobId) {
  const cause = (document.getElementById('revisitCauseSelect')?.value || '').trim();
  const causeDesc = (document.getElementById('revisitCauseDesc')?.value || '').trim();
  const outcome = (document.getElementById('revisitOutcomeSelect')?.value || '').trim();
  const outcomeDesc = (document.getElementById('revisitOutcomeDesc')?.value || '').trim();
  const statusEl = document.getElementById('revisitChecklistStatus');

  if (!cause) { if (statusEl) statusEl.textContent = '⚠️ กรุณาเลือกสาเหตุงานแก้ไขก่อน'; return; }
  if (!outcome) { if (statusEl) statusEl.textContent = '⚠️ กรุณาเลือกผลการแก้ไขก่อน'; return; }

  cwfSaveRevisitChecklist(jobId, { cause, cause_desc: causeDesc, outcome, outcome_desc: outcomeDesc });

  // Try save to server (non-blocking)
  fetch(`${API_BASE}/jobs/${encodeURIComponent(String(jobId))}/revisit-checklist`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ revisit_cause: cause, revisit_cause_desc: causeDesc, revisit_outcome: outcome, revisit_outcome_desc: outcomeDesc }),
  }).catch(() => {});
...
function cwfRefreshChecklistView(jobId, section){
  try {
    const ctx = window.__cwfChecklistContext || null;
    if (ctx && ctx.mode === 'unit' && ctx.unitId) {
      window.__cwfUnitChecklistSection = section || ctx.section || 'pre';
      openTechPhotoModal(ctx.jobId || jobId, 'checklist', ctx.unitId);
      return;
    }
  } catch (_) {}
  openTechChecklistModal(jobId, section);
}
function cwfSetChecklistAll(jobId, section){
  ...
  cwfSaveChecklist(jobId, data); cwfRefreshChecklistView(jobId, section);
}
...
function cwfToggleCheck(jobId, section, idx, checked){
  ...
  row.checked = !!checked; if (checked) row.issue = false; data[section][idx] = row; cwfSaveChecklist(jobId, data); cwfRefreshChecklistView(jobId, section);
}
window.cwfToggleCheck = cwfToggleCheck;
function cwfToggleIssue(jobId, section, idx){
  const data = cwfGetChecklist(jobId); data[section] = data[section] || {}; const row = data[section][idx] || {};
  row.issue = !row.issue; if (row.issue) row.checked = false; data[section][idx] = row; cwfSaveChecklist(jobId, data); cwfRefreshChecklistView(jobId, section);
}
    ...
    <div class="cwf-check-list">
      ${items.map((label,i)=>{ const row = data?.[section]?.[i] || {}; return `
        <div class="cwf-check-row">
          ...
          <div class="cwf-check-tools">
            <button type="button" class="cwf-link-btn" onclick="cwfToggleIssue('${String(jobId).replace(/'/g,"\\'")}', '${section}', ${i})">${row.issue ? 'ยกเลิกปัญหา' : issueText}</button>
            ${row.issue ? `<button type="button" class="cwf-link-btn" onclick="cwfAttachChecklistIssuePhoto('${String(jobId).replace(/'/g,"\\'")}', '${section}', ${i}, '')">📎 แนบรูปปัญหา</button>` : ''}
          </div>
          ${row.issue ? `<div class="cwf-issue-note"><textarea placeholder="หมายเหตุ เช่น บานสวิงหักอยู่ก่อน / หน้ากากแตก / มีน้ำหยด" oninput="cwfSaveIssueNote('${String(jobId).replace(/'/g,"\\'")}', '${section}', ${i}, this.value)">${String(row.note||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</textarea><div class="cwf-check-issue-photo">แนบรูปประกอบได้ เพื่อใช้เป็นหลักฐานว่าปัญหามีอยู่ก่อนหรือพบจากหน้างานจริง</div></div>` : ''}
        </div>`; }).join('')}
    </div>`;
}
function openTechChecklistModal(jobId, section){
  window.__cwfChecklistContext = { mode:'job', jobId:String(jobId), section:section || 'pre' };
  const active = section || 'pre';
  const key = cwfCloseKey(jobId);
  const html = `
...
async function cwfValidateCloseRequirements(jobId, targetStatus){
  ...
  if (units.length) {
    for (const u of units) {
      const label = `เครื่องที่ ${u.unit_no || '-'} รหัส ${u.unit_code || '-'}`;
      if (Number((u.photo_counts || {}).nameplate || 0) < 1) { alert(`${label} ยังไม่มีรูปเนมเพลท`); openTechPhotoModal(jobId, 'photos', u.unit_id); return false; }
      if (Number((u.photo_counts || {}).before || 0) < 1) { alert(`${label} ยังไม่มีรูปก่อนทำ`); openTechPhotoModal(jobId, 'photos', u.unit_id); return false; }
      if (Number((u.photo_counts || {}).after || 0) < 1) { alert(`${label} ยังไม่มีรูปหลังทำ`); openTechPhotoModal(jobId, 'photos', u.unit_id); return false; }
    }
    const checklistOk = await cwfAutoCompleteMissingUnitChecklists(jobId, units);
    if (!checklistOk) { openTechPhotoModal(jobId, 'checklist'); return false; }
    const payMsgUnit = await cwfValidatePayment(jobId); if (payMsgUnit) { alert(payMsgUnit === 'กรุณาเลือกวิธีเก็บเงินลูกค้า' ? 'ยังไม่ได้เก็บเงินกับลูกค้า กรุณาเลือกวิธีชำระเงินก่อนปิดงาน' : payMsgUnit); openTechPaymentModal(jobId); return false; }
    return true;
  }
  ...
  const payMsg = await cwfValidatePayment(jobId); if (payMsg) { alert(payMsg === 'กรุณาเลือกวิธีเก็บเงินลูกค้า' ? 'ยังไม่ได้เก็บเงินกับลูกค้า กรุณาเลือกวิธีชำระเงินก่อนปิดงาน' : payMsg); openTechPaymentModal(jobId); return false; }
...
function buildJobCard(job, historyMode = false) {
  ...
  div.innerHTML = `
    ...
    ${revisitFlow ? `
      <div style="margin-top:10px;background:#fff7ed;border:1.5px solid rgba(234,88,12,0.22);border-radius:18px;padding:14px;">
        <div style="color:#9a3412;font-weight:900;font-size:15px;">🔁 งานแก้ไข / กลับไปตรวจซ้ำ</div>
        <div style="margin-top:6px;color:#7c2d12;font-size:13px;">${escape(revisitReason || "แอดมินส่งงานเดิมกลับมาให้ช่างเข้าดูอาการเพิ่มเติม")}</div>
        <div style="margin-top:10px;display:flex;flex-direction:column;gap:4px;font-size:12.5px;color:#7c2d12;">
          <span>✅ งานนี้ไม่มีเก็บเงินลูกค้า</span>
          <span>✅ ไม่มีค่าตอบแทนเพิ่มเติมสำหรับช่าง</span>
          <span>✅ ไม่ต้องแนบสลิป / ไม่ต้องลงเนมเพลทใหม่</span>
        </div>
        ${canEdit ? `
        <div style="margin-top:14px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          <button type="button" onclick="openRevisitAppointmentModal('${jobKeyJs}')"
            style="border-radius:16px;min-height:58px;background:#fff;border:1.5px solid rgba(234,88,12,0.3);color:#9a3412;font-weight:900;font-size:13px;">
            🕛<br><span style="font-size:12px;">แจ้งเวลานัดหมาย</span>
          </button>
          <button type="button" onclick="openRevisitMediaHub('${jobKeyJs}')"
            style="border-radius:16px;min-height:58px;background:#fff;border:1.5px solid rgba(234,88,12,0.3);color:#9a3412;font-weight:900;font-size:13px;">
            📷✅<br><span style="font-size:12px;">รูปและเช็คลิส</span>
          </button>
        </div>
        ` : ``}
      </div>
    ...
    ${showWorkTools ? `
      <details class="cwf-details" style="margin-top:10px;" ${isWorking ? "open" : ""}>
        <summary>${revisitFlow ? "🔁 ปิดงานแก้ไข" : "🛠️ ปิดงาน / หลักฐาน"}</summary>
        <div class="cwf-details-body">
          ${!revisitFlow ? `
          ...
          <div class="cwf-note-box">
            <b>📝 หมายเหตุช่าง${revisitFlow ? " (ไม่บังคับ)" : ""}</b>
            <textarea id="note-${keyBase}" rows="${revisitFlow ? 2 : 3}" style="margin-top:6px;" placeholder="${revisitFlow ? "บันทึกเพิ่มเติม ถ้ามี" : "เจอปัญหาอะไร ใส่ไว้ได้"}" ${!canEdit ? "disabled" : ""} oninput="noteDraftChanged('${jobKeyJs}')">${escape(getNoteDraft(keyBase) || job.technician_note || "")}</textarea>
...
function cwfTodayIso(){ return cwfIsoDateLocal(new Date()); }
function cwfMonthText(d){ return cwfIsoDateLocal(d || new Date()).slice(0,7); }
...
function cwfFormatMonthTitle(month){
  try{
    const d = new Date(String(month || cwfMonthText()) + '-01T00:00:00');
    return d.toLocaleDateString('th-TH', { month:'long', year:'numeric' });
  }catch{ return month || cwfMonthText(); }
}
...
function ensureWorkdaysModal(){
    ...
    <div class="cwf-calendar-panel" role="dialog" aria-modal="true" aria-labelledby="cwfWorkCalendarTitle">
      <div class="cwf-calendar-head">
        <div>
          ...
          <p>ใช้สำหรับงานล่วงหน้าที่แอดมินมอบหมายเท่านั้น</p>
          <span class="cwf-urgent-pill">งานด่วนใช้ปุ่มหน้าแรก</span>
        </div>
        <button class="cwf-close" type="button" id="techWorkCalendarClose" aria-label="ปิด">ปิด</button>
      </div>

      <div class="cwf-calendar-toolbar">
        <button type="button" class="cwf-month-nav" id="calendarPrevMonth" aria-label="เดือนก่อน">‹</button>
        <button type="button" class="cwf-month-title" id="workCalendarMonthLabel">เดือนนี้</button>
        <input id="workCalendarMonth" type="month" class="cwf-month-input" aria-label="เลือกเดือน">
        <button type="button" class="cwf-month-nav" id="calendarNextMonth" aria-label="เดือนถัดไป">›</button>
      </div>
      ...
      <div class="cwf-calendar-help">
        ...
        <span>❌ ไม่รับ</span>
        <span>📌 มีงานแล้ว</span>
        <span>⚙️ ค่าพิเศษ</span>
      </div>

      <div class="cwf-calendar-layout">
        <div class="cwf-calendar-left">
          <div class="cwf-action-title">ตั้งค่าหลายวัน</div>
          <div class="cwf-calendar-actions">
            <button type="button" class="cwf-chip-btn" id="toggleMultiSelectBtn">เลือกหลายวัน</button>
            <button type="button" class="cwf-chip-btn" id="selectAllMonthBtn">เลือกทั้งเดือน</button>
            <button type="button" class="cwf-chip-btn" id="selectWeekdaysBtn">จันทร์-ศุกร์</button>
            <button type="button" class="cwf-chip-btn" id="selectWeekendBtn">เสาร์-อาทิตย์</button>
            <button type="button" class="cwf-chip-btn subtle" id="copyPrevMonthBtn">📋 เหมือนเดือนก่อน</button>
            <button type="button" class="cwf-chip-btn danger" id="clearSelectedDatesBtn">ล้างที่เลือก</button>
          </div>
        ...
        <div class="cwf-day-editor">
          ...
          <div class="cwf-quick-grid">
            <button id="singleOpenDayBtn" class="cwf-save-btn" type="button">✅ เปิดรับ</button>
            <button id="singleCloseDayBtn" class="cwf-soft-btn danger" type="button">❌ ปิดรับ</button>
            <button id="editSelectedDayBtn" class="cwf-soft-btn full" type="button">✏️ ตั้งค่าพิเศษ</button>
          </div>
  ...
  style.textContent = `
    .cwf-calendar-backdrop{position:fixed!important;inset:0!important;display:none;align-items:flex-start;justify-content:center;z-index:99999!important;background:linear-gradient(180deg,rgba(15,23,42,.72),rgba(15,23,42,.58))!important;padding:10px;overflow:auto;backdrop-filter:blur(8px)}
    .cwf-calendar-panel{width:min(920px,100%);background:linear-gradient(180deg,#f8fbff 0%,#eef6ff 48%,#f8fafc 100%)!important;border:1px solid rgba(191,219,254,.9);border-radius:22px;box-shadow:0 28px 90px rgba(15,23,42,.36),0 1px 0 rgba(255,255,255,.85) inset;padding:10px;color:#0f2f5f;opacity:1!important}
    .cwf-calendar-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(232,242,255,.96));color:#0f2f5f;border:1px solid rgba(191,219,254,.9);border-radius:18px;padding:12px 12px;box-shadow:0 12px 34px rgba(15,47,95,.1),0 1px 0 rgba(255,255,255,.9) inset}
    .cwf-calendar-head h3{margin:0;font-size:18px;line-height:1.2;font-weight:950;letter-spacing:0}.cwf-calendar-head p{margin:3px 0 0;color:#64748b;font-size:12px;font-weight:850;line-height:1.35}.cwf-eyebrow{display:none}.cwf-urgent-pill{display:inline-flex;margin-top:7px;border:1px solid #fde68a;background:#fff7c2;color:#695200;border-radius:999px;padding:4px 8px;font-size:10px;font-weight:950;line-height:1}
    .cwf-close{border:1px solid rgba(203,213,225,.9);background:rgba(255,255,255,.9);color:#334155;border-radius:999px;min-width:36px;min-height:32px;padding:0 11px;font-size:12px;font-weight:950;box-shadow:0 8px 18px rgba(15,23,42,.08)}
    .cwf-calendar-toolbar{display:grid;grid-template-columns:42px minmax(0,1fr) 42px;gap:8px;align-items:center;margin:11px 0}.cwf-month-nav{border:1px solid rgba(191,219,254,.95);background:#fff;color:#0f2f5f;border-radius:14px;min-height:40px;font-size:24px;font-weight:900;line-height:1;box-shadow:0 8px 18px rgba(15,47,95,.08)}.cwf-month-title{border:1px solid rgba(191,219,254,.95);background:linear-gradient(180deg,#fff,#f6fbff);color:#0f2f5f;border-radius:14px;min-height:40px;padding:8px 10px;font-size:15px;font-weight:950;box-shadow:0 8px 18px rgba(15,47,95,.07)}.cwf-month-input{position:absolute;opacity:0;pointer-events:none;width:1px;height:1px}
    .cwf-calendar-help{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0}.cwf-calendar-help span{background:#fff;border:1px solid #e2e8f0;border-radius:999px;padding:5px 9px;font-size:11px;font-weight:900;color:#334155;line-height:1;box-shadow:0 6px 14px rgba(15,23,42,.04)}.cwf-calendar-help span:nth-child(1){background:#ecfdf3;border-color:#bbf7d0;color:#166534}.cwf-calendar-help span:nth-child(2){background:#fff5f5;border-color:#fecaca;color:#991b1b}.cwf-calendar-help span:nth-child(3){background:#eff6ff;border-color:#bfdbfe;color:#1d4ed8}.cwf-calendar-help span:nth-child(4){background:#fffbeb;border-color:#fde68a;color:#92400e}
    .cwf-calendar-layout{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(280px,.72fr);gap:10px}.cwf-calendar-left{min-width:0}.cwf-action-title{font-size:11px;font-weight:950;color:#64748b;margin:5px 2px 7px}.cwf-calendar-actions{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:8px}.cwf-chip-btn{border:1px solid rgba(203,213,225,.9);background:rgba(255,255,255,.94);color:#0f2f5f;border-radius:999px;min-height:35px;padding:7px 9px;font-size:12px;font-weight:950;box-shadow:0 7px 16px rgba(15,47,95,.05)}.cwf-calendar-actions .cwf-chip-btn:first-child{background:linear-gradient(135deg,#0b62d6,#0891b2);color:#fff;border-color:transparent;box-shadow:0 10px 22px rgba(11,98,214,.22)}.cwf-chip-btn.subtle{background:#fff8d8;border-color:#fde68a;color:#695200}.cwf-chip-btn.danger{background:rgba(255,255,255,.95);color:#b91c1c;border-color:#fecaca;box-shadow:none}.cwf-multi-hint{background:#fff8d8;border:1px solid #ffe08a;border-radius:14px;padding:8px 10px;margin-bottom:8px;color:#513f00;font-size:12px;font-weight:900;box-shadow:0 8px 20px rgba(250,204,21,.08)}
    .cwf-week-head{display:grid;grid-template-columns:repeat(7,1fr);gap:4px;margin-bottom:4px}.cwf-week-head span{text-align:center;font-size:11px;font-weight:900;color:#64748b}
    .cwf-month-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:5px}.cwf-day-cell{height:56px;border:1px solid rgba(203,213,225,.9);background:rgba(255,255,255,.96);border-radius:15px;padding:6px 3px;text-align:center;color:#0f2f5f;box-shadow:0 8px 18px rgba(15,47,95,.055);position:relative;overflow:hidden;font-weight:950;display:grid;align-content:center;gap:2px;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.cwf-day-cell:active{transform:scale(.97)}.cwf-day-cell.is-selected{border-color:#0b62d6;box-shadow:0 0 0 2px #facc15,0 12px 24px rgba(11,98,214,.18)}.cwf-day-cell.is-picked{box-shadow:0 0 0 2px rgba(14,165,233,.28) inset,0 8px 18px rgba(15,47,95,.055)}.cwf-day-cell.is-off{background:linear-gradient(180deg,#fff,#fff5f5);border-color:#fecdd3}.cwf-day-cell.is-advance{background:linear-gradient(180deg,#fff,#ecfdf3);border-color:#bbf7d0}.cwf-day-cell.is-custom{background:linear-gradient(180deg,#fffdf3,#fffbeb);border-color:#facc15}.cwf-day-cell.is-custom:after{content:"";position:absolute;left:9px;right:9px;bottom:3px;height:2px;border-radius:999px;background:#facc15}.cwf-day-cell.is-locked{background:linear-gradient(180deg,#f8fbff,#eaf2ff);border-color:#93c5fd;color:#0f2f5f}.cwf-day-num{font-size:16px;line-height:1}.cwf-day-label{font-size:10px;line-height:1;color:#475569}.cwf-day-icons{font-size:10px;line-height:1;min-height:10px;letter-spacing:0}.cwf-job-dot{position:absolute;right:4px;top:4px;background:#1d4ed8;color:#fff;border-radius:999px;padding:1px 5px;font-size:9px;font-weight:950;box-shadow:0 5px 12px rgba(29,78,216,.22)}
    .cwf-legend-row{display:none}
    .cwf-day-editor{background:rgba(255,255,255,.96);border:1px solid rgba(191,219,254,.75);border-radius:18px;padding:11px;display:grid;gap:10px;box-shadow:0 16px 36px rgba(15,47,95,.1);align-self:start}.cwf-editor-title{display:flex;justify-content:space-between;gap:8px;align-items:center;background:linear-gradient(180deg,#f8fbff,#eef6ff);border:1px solid rgba(219,234,254,.9);border-radius:14px;padding:9px;font-size:13px;font-weight:950}.cwf-editor-title b{background:#eaf2ff;color:#1d4ed8;border-radius:999px;padding:5px 9px;font-size:11px;white-space:nowrap}.cwf-day-summary{background:linear-gradient(180deg,#fff,#f8fbff);border:1px solid rgba(219,234,254,.9);border-radius:15px;padding:9px;font-size:12px;font-weight:800;line-height:1.45;color:#334155}.cwf-detail-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.cwf-detail-grid div{background:#fff;border:1px solid #e2e8f0;border-radius:13px;padding:8px;box-shadow:0 6px 14px rgba(15,23,42,.035)}.cwf-detail-grid span{display:block;font-size:10px;color:#64748b;font-weight:900}.cwf-detail-grid b{display:block;margin-top:3px;font-size:12px;color:#0f2f5f;line-height:1.35}.cwf-detail-note{grid-column:1/-1}.cwf-section-title{font-weight:950;color:#0f2f5f;margin:2px 0;font-size:13px}.cwf-edit-panel{border:1px solid rgba(219,234,254,.9);background:linear-gradient(180deg,#f8fbff,#fff);border-radius:15px;padding:10px;display:grid;gap:9px}.cwf-time-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.cwf-day-editor label{font-size:12px;font-weight:900;color:#334155;display:grid;gap:4px}.premium-input{width:100%;border:1px solid #d7e0ee;border-radius:12px;min-height:39px;padding:8px 10px;background:#fff;color:#0f2f5f;font-weight:850;box-sizing:border-box}.cwf-quick-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.cwf-save-btn,.cwf-soft-btn{border-radius:13px;min-height:40px;padding:9px 10px;font-size:13px;font-weight:950;transition:transform .16s ease,box-shadow .16s ease}.cwf-save-btn:active,.cwf-soft-btn:active{transform:scale(.98)}.cwf-save-btn{border:0;background:linear-gradient(135deg,#0b62d6,#0891b2);color:#fff;box-shadow:0 10px 22px rgba(11,98,214,.2)}.cwf-soft-btn{background:#fff;border:1px solid #d7e0ee;color:#0f2f5f;box-shadow:0 7px 16px rgba(15,47,95,.05)}.cwf-soft-btn.danger{color:#b91c1c;border-color:#fecaca;background:#fff;box-shadow:none}.cwf-save-btn:disabled,.cwf-soft-btn:disabled{opacity:.45;filter:grayscale(.2);box-shadow:none}.full{width:100%;grid-column:1/-1}.cwf-calendar-status{font-size:11px;font-weight:850;color:#64748b;margin-top:7px;min-height:16px}
    @media(max-width:820px){.cwf-calendar-backdrop{padding:8px}.cwf-calendar-panel{border-radius:16px;padding:9px}.cwf-calendar-layout{grid-template-columns:1fr}.cwf-calendar-actions{grid-template-columns:1fr 1fr}.cwf-day-editor{order:2}.cwf-day-cell{height:50px;border-radius:9px}.cwf-calendar-head{border-radius:12px}.cwf-time-grid{grid-template-columns:1fr 1fr}.cwf-calendar-help span{font-size:10px;padding:5px 7px}}
  `;
  document.head.appendChild(style);
  ...
  document.getElementById('workCalendarMonthLabel').onclick = () => {
    const input = document.getElementById('workCalendarMonth');
    try{ if(input?.showPicker) input.showPicker(); else input?.focus(); }catch{ input?.focus(); }
  };
  ...
  document.getElementById('singleOpenDayBtn').onclick = () => setSelectedDayAdvance(true);
  document.getElementById('singleCloseDayBtn').onclick = () => setSelectedDayAdvance(false);
...
async function loadWorkdaysModalData(month){
  ...
  const monthLabel = document.getElementById('workCalendarMonthLabel'); if(monthLabel) monthLabel.textContent = cwfFormatMonthTitle(m);
...
function selectWorkCalendarDate(iso, rerender=true){
  ...
  if(summary){
    const statusText = locked ? `📌 มีงานแล้ว ${cwfDayJobCount(iso)} งาน` : (it.can_accept_advance_job ? '✅ รับงานล่วงหน้า' : '❌ ไม่รับงานล่วงหน้า');
...
async function setSelectedDayAdvance(isAdvance){
  const iso = __cwfWorkCalendarState.selectedDate;
  if(!iso) return;
  if(cwfIsLockedDay(iso)){ cwfLockedPopup(iso); return; }
  const current = cwfEffectiveCalendarItem(iso);
  if(!!current.can_accept_advance_job === !!isAdvance){
    selectWorkCalendarDate(iso, true);
    return;
  }
  const body = buildDayPayload(iso, !!isAdvance, isAdvance ? {} : { note: current.note || '' });
  try{
...
function updateMultiModeUI(){
  const btn = document.getElementById('toggleMultiSelectBtn'); if(btn) btn.textContent = __cwfWorkCalendarState.multiMode ? 'เลิกเลือกหลายวัน' : 'เลือกหลายวัน';
  ...
  if (targetStatus === 'เสร็จแล้ว') {
    (async () => {
      try {
        ...
        const revisitFlow = isRevisitJob(job);
        if (revisitFlow) {
          const readyCheck = await checkRevisitCloseReady(jobId);
          if (!readyCheck.ok) {
            alert(readyCheck.msg);
            return;
          }
        }
...
async function finalizeJob(jobId, targetStatus, signatureDataUrl) {
  try {
    const job = getJobFromCache(jobId);
    const revisitFlow = isRevisitJob(job);
    const revisitCl = revisitFlow ? cwfGetRevisitChecklist(jobId) : {};
    const revisit_result = revisitCl.outcome || "";
    const revisit_note = [revisitCl.cause_desc, revisitCl.outcome_desc].filter(Boolean).join(' | ') || "";
    const revisit_cause = revisitCl.cause || "";
    const revisit_outcome = revisitCl.outcome || "";
    const revisit_cause_desc = revisitCl.cause_desc || "";
    const revisit_outcome_desc = revisitCl.outcome_desc || "";
