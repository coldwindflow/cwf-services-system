...
if [ ! -f "$BASE_DIR/admin-ai-office.html" ] || [ ! -f "$BASE_DIR/admin-ai-office.js" ]; then
  echo "Cannot find v5 source files. Run this script from the extracted zip folder or pass repo root." >&2
  exit 1
fi
...
print('AI Office v5 backend patched: admin session only, no extra PIN, conversation history enabled.')
PY
...
if p.exists():
    ...
    old = s
    import re
    s = re.sub(r'CACHE_NAME\s*=\s*"[^"]+"', 'CACHE_NAME = "cwf-cache-ai-office-v5-20260607"', s, count=1)
    if '/admin-ai-office.js' in s and 'admin-ai-office-production-v5' not in s:
        pass
    if s != old:
        p.write_text(s, encoding='utf-8')
...
echo "Done. Commit, deploy, then hard refresh /admin/ai-office."
